...
# Imports for the IceDtaxDescriptor
...
class Collector:
    ...
    def __init__(
        ...
        try:
            ...
            self.icepap_system = IceDtaxDescriptor(icepap_controller, hostname)
        except Exception as e:
            msg = (
                "Failed to instantiate master controller.\nHost: "
                "{}\nPort: {}\nAlt hostname:{}\n{}".format(
                    icepap_controller.host, icepap_controller.port, hostname, e
                )
            )
            raise Exception(msg)

        if not self.icepap_system:
            msg = "IcePAP system {} has no active drivers! " "Aborting.".format(
                icepap_controller.host
            )
            raise Exception(msg)
