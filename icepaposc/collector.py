...
class IcePBoxDescriptor(IcePAPDescriptor):
    ...
    def __init__(self, host, port, timeout):
        ...
        # Update getters and siglist with pandabox
        self.sig_getters.update(
            [('PboxEnc', self._getter_pbox_enc_val),
            ]
        )
        self.sig_list = list(self.sig_getters.keys())
        # Get the pandabox
        # self.pandabox = BlockingClient(self.host2)
        # self.pandabox.connect()

    def _getter_pbox_enc_val(self, addr):
        comm = 'INENC' + str(addr) + '.VAL'
        val = float(self.pandabox.send(Get(comm)))
        return val
